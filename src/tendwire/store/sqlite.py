    ...
    *,
    ...
    for item in prepared_turn_items:
        ...
        existing_turn = conn.execute(
            """
            SELECT payload_json, worker_fingerprint
            FROM turns
            WHERE host_id = ? AND turn_id = ?
            """,
            (host_id, turn_id),
...
def _snapshot_owned_turn_candidate_ref_conn(
    ...
    candidates = [
        ...
        for turn_id, worker_fingerprint, source_turn_id, origin_command_id in rows
    ]
    expected_origin = str(projection.get("origin_command_id") or "").strip()
    if expected_origin:
        source_rows = [
            row for row in candidates if row[2] and row[3] == expected_origin
        ]
        if len(source_rows) > 1:
            raise StoreSchemaError("turn_owner_source_ambiguous")
        if source_rows:
            return source_rows[0][0], source_rows[0][1]
        command_rows = [
            row for row in candidates if not row[2] and row[3] == expected_origin
        ]
        if len(command_rows) > 1:
            raise StoreSchemaError("turn_owner_command_ambiguous")
        if command_rows:
            return command_rows[0][0], command_rows[0][1]
        return None
    placeholder_rows = [row for row in candidates if not row[2] and not row[3]]
    if len(placeholder_rows) > 1:
        raise StoreSchemaError("turn_owner_placeholder_ambiguous")
    if not placeholder_rows:
        return None
    return placeholder_rows[0][0], placeholder_rows[0][1]
