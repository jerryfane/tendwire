...
def _adopted_command_canonical_turn_id(payload: Mapping[str, Any]) -> str | None:
    """Derive an adopted row's source identity only when its owner is stable."""
    if (
        str(payload.get("source") or "") != "command"
        or not str(payload.get("source_turn_id") or "").strip()
        or _turn_continuity_identity(payload) is None
    ):
        return None
    canonical = dict(payload)
    canonical.pop("origin_command_id", None)
    canonical.pop("superseded_by_turn_id", None)
    canonical.pop("superseded_at", None)
    return str(Turn.from_dict(canonical).id)
...
def _linked_canonical_turn_id_conn(
    ...
    while current and current not in seen:
        ...
        if not str(payload.get("source_turn_id") or "").strip():
            return None
        if str(payload.get("source") or "") == "command":
            return _adopted_command_canonical_turn_id(payload)
        return current
    return None
        ...
        while True:
            ...
            for turn_id, payload_json, observed_at, _sequence in rows:
                ...
                if _turn_is_tombstoned(payload):
                    ...
                    canonical_turn_id = _adopted_command_canonical_turn_id(payload)
