...
def test_v20_backfill_production_shape_is_lossless_idempotent_and_fail_closed(
    ...
    with sqlite3.connect(str(db_path)) as conn:
        ...
        adopted_payload = json.loads(
            conn.execute(
                "SELECT payload_json FROM turns WHERE host_id = ? AND turn_id = ?",
                ("host-a", str(adopted["id"])),
            ).fetchone()[0]
        )
        adopted_canonical_id = store_sqlite._adopted_command_canonical_turn_id(
            adopted_payload
        )
        assert adopted_canonical_id is not None
        ...
        assert aliases == sorted(
            [
                (str(adopted["id"]), adopted_canonical_id),
                (str(duplicate["id"]), observed_turn_id),
            ]
        )
