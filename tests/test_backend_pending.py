...
def test_current_v10_creation_has_exact_binding_and_claim_state(tmp_path: Path) -> None:
    db = tmp_path / "current-v10.db"
    init_store(db)
    with sqlite3.connect(db) as conn:
        assert conn.execute("PRAGMA user_version").fetchone()[0] == STORE_SCHEMA_VERSION == 10
        columns = {
            str(row[1])
